  ...
  <form class="tags-setup__form" @submit.prevent="onSubmit" v-if="view === 'welcome'" autocomplete="off">
    ...
    <div class="tags-list__wrapper">
      <draggable v-model="steemTags" v-if="chain === CHAINS.STEEM">
        <span class="tags-list__item tags-list__item--removable" v-for="tag in steemTags" :key="tag.text"><span>{{tag.text | unGolosTag | toLowerCase}}</span>
          <span class="tags-list__remove-item" @click="removeTag(tag)"></span>
        </span>
      </draggable>
      <draggable v-model="golosTags" v-if="chain === CHAINS.GOLOS">
        <span class="tags-list__item tags-list__item--removable" v-for="tag in golosTags" :key="tag.text"><span>{{tag.text | unGolosTag | toLowerCase}}</span>
          <span class="tags-list__remove-item" @click="removeTag(tag)"></span>
        </span>
      </draggable>
    </div>
    ...
    <div class="settings__tags-panel" :class="{'settings__tags-panel--active': chain === CHAINS.STEEM}">
      ...
      <div class="tags-list__wrapper">
        <draggable v-model="steemTags">
          <span class="tags-list__item tags-list__item--removable" v-for="tag in steemTags" :key="tag.text"><span>{{tag.text}}</span>
            <span class="tags-list__remove-item" @click="removeTag(tag)"></span>
          </span>
        </draggable>
      </div>
