...
module.exports = ({large = true, highQualityPost = true, noImage = false, sanitizeErrors = []}) => ({
  allowedTags,
  // figure, figcaption,
  ...
  transformTags: {
    ...
    div: (tagName, attribs) => {
      const attys = {}
      const classWhitelist = ['pull-right', 'pull-left', 'text-justify', 'text-rtl', 'text-center', 'text-right', 'video-wrapper']
      const validClass = classWhitelist.find(e => attribs.class == e)
      if (validClass)
        attys.class = validClass

      if (attribs['data-src'])
        attys['data-src'] = attribs['data-src']
      return {
        tagName,
        attribs: attys
      }
